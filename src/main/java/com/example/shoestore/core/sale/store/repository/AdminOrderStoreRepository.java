...
import com.example.shoestore.core.sale.store.model.response.CartResponse;
import com.example.shoestore.core.sale.store.model.response.DeliveryResponse;
import com.example.shoestore.core.sale.store.model.response.OrderStatusResponse;
...
@Repository
public interface AdminOrderStoreRepository extends OrdersRepository {
    @Query(nativeQuery = true, value = """
            ...
            WHERE co.`is_deleted` = :isDeleted
            AND co.`status` = :status
            AND (co.`code` LIKE :codeOrder OR :codeOrder IS NULL)
            """)
    ...
    @Query(nativeQuery = true, value = """
            SELECT `customer_order`.`id`,
                ...
                `customer_order`.`voucher_id`,
                `customer_order`.`payment_method_id`,
                `customer_order`.`code`,
                `customer_order`.`name`,
                `customer_order`.`total_money`,
                `customer_order`.`total_payment`,
                `customer_order`.`date_payment`,
                `customer_order`.`shipping_price`,
                `customer_order`.`shipping_date`,
                `customer_order`.`received_date`,
                `customer_order`.`created_time`,
                `customer_order`.`updated_time`,
                `customer_order`.`status`,
                `customer_order`.`created_by`,
                `customer_order`.`updated_by`,
                `customer_order`.`is_delete`,
                `customer_order`.`customer_ordercol`
            FROM `database_final_datn_g33321`.`customer_order`
            WHERE  `customer_order`.`status` = :status 
            AND `customer_order`.`created_time` = :date
            """)
    List<Order> findListExport(@Param("status") int status, @Param("date") LocalDate date);
    ...
    @Query(nativeQuery = true, value = """
            ...
            WHERE `id` = :idOrder;
                        """)
    void udpateTotalMoney(@Param("totalMoney") BigDecimal totalPrice, @Param("idOrder") Long idOrder);
